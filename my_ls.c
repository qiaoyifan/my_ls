/*************************************************************************
	...
	> Created Time: 2017年05月12日 星期五 11时43分34秒
 ************************************************************************/
...
void dislpay_attribute(struct stat buf,char * name)
{
    char        buf_time[32];
    struct passwd   *psd;   //从该结构体中获取文件名所有者的用户名
    struct group   *grp;   //从该结构体中获取文件名所有者所属组的组名
 ...
 *   根据命令行参数好完整路径名显示目标文件
 *   参数flag:  命令行参数
 *   参数pathname:  办好了文件ing的路径名
 */
 void dislpay(int flag ,char * pathname)
 {
     int         i,j;
     struct stat   buf;
 ...
 void dislpay_dir(int flag_param, char *path)
 {
     DIR               *dir;
     struct dirent     *ptr;
     int        count = 0;
    ...
    //获取该目录下文件总数和最长的文件名
     ...
     //使用冒泡发对文件名进行排序，排序后文件名按字母顺序存储于filenames
     for(i = 0; i < count-1; i++)
     {
    ...
    for(i = 0; i < count; i++)
    {
        dislpay(flag_param, filenames[i]);
    }

    closedir(dir);

    //如果命令行中没有没有-l选项，打印一个换行符
 ...
 int main(int argc, char ** argv)
 {
     ...
     if((num+1) == argc)
     {
         strcpy(path, "./");
         path[2] = '\0';
         dislpay_dir(flag_param, path);
         return 0;
     }

     i = 1;
     do{
        //如果不是目标文件夹或目录，解析下一个命令行参数
        if(argv[i][0] == '-')
         {
             i++;
             continue;
